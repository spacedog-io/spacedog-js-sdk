let SpaceDog = require('../spacedog.min.js');
let assert = require('assert');
let expect = require('chai').expect;
let xhrMock = require('xhr-mock');
let localStorageMock = require('./util-mockLocalStorage.js');
...
describe('credentials # ', function () {
    ...
    it('should say false by default on canTryLogin', function () {
        ...
        let canTryLogin = SpaceDog.Credentials.canTryLogin();
    ...
    it('should get the enabled state of a user', function (done) {

        xhrMock.get('https://dummyBackendId.spacedog.io/1/credentials/dummyCredentialId', function (req, res) {
            return res.status(200).header('Content-Type', 'application/json').body(JSON.stringify({
                enabled: false
            }));
        });
        ...
        SpaceDog.Credentials.getEnabledState({
            credentialId: "dummyCredentialId",
        }, function (err, res) {
            expect(err).to.be.null;
    ...
    it('should set the enabled state of a user', function (done) {

        xhrMock.put('https://dummyBackendId.spacedog.io/1/credentials/dummyCredentialId/enabled', function (req) {
            expect(JSON.parse(req.body())).to.be.false;
        });

        SpaceDog.Credentials.setEnabledState({
            credentialId: "dummyCredentialId",
            state: false
        }, function () {
            done()
